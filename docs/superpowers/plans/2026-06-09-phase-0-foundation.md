...
1. **Env contract matches the real `.env`.** Actual `.env` has `VITE_APPWRITE_PROJECT_ID`, `VITE_APPWRITE_PROJECT_NAME`, `VITE_APPWRITE_ENDPOINT`, `APPWRITE_API_KEY`. Scripts read `APPWRITE_PROJECT_ID ?? VITE_APPWRITE_PROJECT_ID` (same for endpoint/name). The secret `APPWRITE_API_KEY` has no `VITE_` fallback and no `VITE_` variant may exist â€” checked at runtime.
2. **Native relationships (GA) instead of string FKs** for entity links: `plantsâ†’species`, `plantsâ†’user_locations`, `observationsâ†’plants`, `treatmentsâ†’observations`, `measurementsâ†’observations`, `photosâ†’observations`, `environment_snapshotsâ†’plants`, `environment_snapshotsâ†’observations`. `user_id` stays a plain string everywhere (Auth users are not TablesDB rows). `public_observations` keeps plain string columns only (export independence; no linkage surface).
...
4. **Required+default conflict resolution** (Appwrite forbids defaults on required columns): columns the docs mark "required with default" become optional-with-default (`preferred_units`, `public_contribution_default`, `contribute_to_public_dataset`, `exif_stripped`, `allow_public_image`, `status`). True requireds keep `required: true` and no default. Relationship columns cannot be `required` in Appwrite â€” `observations.plant_id` requiredness is an app-layer rule.
...
- [x] **Step 1.1:** Scaffold into the existing repo: `npm create vite@latest . -- --template react-ts` (files land alongside docs; do not overwrite `.gitignore` blindly â€” keep the existing one, ensure it still ignores `.env`, `.env.*` except `.env.example`, `node_modules/`, `dist/`, `coverage/`).
...
- [x] **Step 3.2:** Run â€” if anything fails, fix `appwrite/schema.ts` (the schema is wrong, not the test). All green â†’ commit `test: public export privacy boundaries`.
...
- [x] **Step 5.1:** Implement with this exact algorithm (get-then-create, never delete):
  ...
  - Relationship columns are created **only from the child side**; the `twoWayKey` appears on the parent automatically â€” `ensureColumn` for relationship kind must also treat "already exists on parent via twoWay" as satisfied.
  ...
  - `ensureIndex`: `getIndex`; on 404 `createIndex({type: IndexType.Key|Unique, columns, ...})`; mismatch â†’ `ManualMigrationRequired`. Then poll `listIndexes` until `available`.
  ...
  - Order: database â†’ all tables â†’ scalar columns (all tables) â†’ wait â†’ relationship columns (children after parents exist) â†’ wait â†’ indexes â†’ wait â†’ buckets.
  - 404 detection: `AppwriteException` with `code === 404`; helper `isNotFound(e)`.
  - Log resource IDs and actions (`created`/`exists`/`skipped`) only â€” never env values.
...
- [x] **Step 6.2:** Run â€” FAIL. Implement `appwrite/seed-data.ts`: export `SEED_USER_IDS` and `SEED_ROWS: SeedRow[]` (`{ tableId, rowId, data, permissions }`), in dependency order:
  ...
  - 3 species (Monstera deliciosa, Epipremnum aureum, Coffea arabica â€” family/genus filled).
  - 4 plants (2 per user; relationship fields set to seed row IDs: `species_id: 'seed_species_monstera'`, `location_id: 'seed_loc_alex_home'`, â€¦).
  ...
  - Child rows: 4 treatments (watering 250 ml top-water, fertilizing 5 ml, repotting, pest_control neem), 3 measurements (height/leaf_count/health_score), 1 photo metadata row (`private_file_id: 'seed_file_placeholder_001'`, `exif_stripped: true`, `allow_public_image: false` â€” no real upload), 2 environment_snapshots (`source: 'manual'` / `'weather_api'`, temps/humidity).
...
- [x] **Step 7.1:** `npm run appwrite:check` â†’ exit 0.
- [x] **Step 7.2:** `npm run appwrite:setup` â†’ database, 10 tables, columns, indexes, 3 buckets created. Re-run immediately â†’ all `exists`, zero creates, exit 0 (idempotency proof).
- [x] **Step 7.3:** `npm run appwrite:seed` â†’ all rows upserted. Re-run â†’ still succeeds, no duplicates (verify by listing row counts).
...
- [x] **Step 9.1:** Run all gates: `npm run lint`, `npm run build`, `npm run test`, `npm run appwrite:check` â€” all green, paste outputs into the session.
- [x] **Step 9.2:** `git status` clean working tree after final commit.

## Self-Review Notes

- Spec coverage: baseline (T1), schema defs (T2), validation tests (T2/T3/T4/T6), setup automation (T5), seed automation (T6), remote verification spec section (T7), docs-update requirements (T8), verification commands (T9). Credential contract adapted to the real `.env` (Locked-In Decision 1) â€” intentional spec deviation, documented.
