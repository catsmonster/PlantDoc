...
- Run public export generation, image processing, and persistent model jobs in Appwrite Functions or a trusted server job. Browser-direct location/weather enrichment is accepted by ADR-007 while the providers remain keyless and no secret is involved; do not turn public Nominatim into autocomplete or bulk geocoding. The transient Gemini preview is accepted by ADR-010 only through the Cloudflare Worker proxy because it needs a server-side API key.
